...
function InterlocutorView(props) {
    const viewClasses = ['absolute w-[20rem] top-[1rem] z-30 py-[1rem] bg-darkSpecial shadow-md shadow-iconsColor text-mainTextColor flex flex-col justify-start items-center space-y-2 rounded-md', props.theState ? 'showDetails' : 'hideDetails'];
    return (
        <div className={viewClasses.join(' ')}>
            ...
            <div className='w-[90%] bg-black rounded-lg px-4 flex flex-col justify-start text-sm py-[2px]'>
                <p>{props.interlocutor.status}</p>
                <p className='text-gray-600'>Joined on 12 june 2022</p>
            </div>
        </div>
    );
}
